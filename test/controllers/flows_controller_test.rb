...
class FlowsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create flow" do
    assert_difference("Flow.count") do
      post flows_url, params: { flow: { description: @flow.description, file: @flow.file, title: @flow.title } }
    end
  ...
  test "should update flow" do
    patch flow_url(@flow), params: { flow: { description: @flow.description, file: @flow.file, title: @flow.title } }
